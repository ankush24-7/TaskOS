    ...
    <div className="w-full flex flex-col justify-center px-3 py-2 pb-2.5 mt-1 rounded-2xl bg-prim-black/60">
      <div className="w-full flex justify-between items-center">
        <span className="flex gap-2.5 items-end">
          ...
          <p className="text-sm h-fit px-2 py-0.5 rounded-2xl leading-none border border-white text-white">
            {status}
          </p>
        </span>
        <span className="flex gap-4 items-center">
          <Link to={`/projects/${_id}/dashboard`} >
            <projectCardIcons.ExternalLink className="w-5 h-5 stroke-white" />
          </Link>
          <button onClick={() => setExpanded(!expanded)} className="cursor-pointer">
            <projectCardIcons.ChevronDown 
              className={`w-7 h-7 stroke-white transition-transform duration-200 ease-in-out ${expanded && " rotate-180"}`} 
            />
          </button>
        </span>
      </div>
          ...
          className="w-full flex flex-col">
            ...
            <span className="flex justify-between items-center mt-2.5">
              <span className="flex items-start gap-1">
                ...
                <p className="leading-none text-white">{dateUtils.formatProjectDate(updatedAt)}</p>
              </span>
              {deadline && (
