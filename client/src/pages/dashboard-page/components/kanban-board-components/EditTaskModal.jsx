...
function HeaderItem(Icon) {
  return (
    <button className="hover:bg-gray-200 p-1.5 rounded-full">{ Icon }</button>
  );
}
        ...
        <div className="flex divide-x-[1px] h-full">
          <div className="flex flex-col w-[75%] pt-4">
            <div className="flex px-4">
              <button>
                {task
                  ...
                  : React.createElement(priorityIcons[3])}
              </button>
              {task ? (
                <input
                  type="text"
            ...
            <input
              ...
              className="font-light mx-4 mt-2 rounded-md px-3 py-1 focus:outline-none focus:bg-[#18181a15] hover:bg-[#18181a15]"
            />
          </div>
          <div className="flex flex-col grow items-center divide-y-[1px]">
            {rightSectionItem(<modalIcons.Calander />, "Due Date")}
            {rightSectionItem(
              <modalIcons.Timeline stroke="#71717a" />,
              "Schedule Task"
            )}
